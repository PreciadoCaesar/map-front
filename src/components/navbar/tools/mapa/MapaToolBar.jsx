...
import { setMeasureMode } from '@/redux/features/mapSlice';
...
export const MapaToolBar = () => {
  ...
  const handleToggle = (label) => {
    ...
    // Borrar - eliminar TODAS las geometrías del mapa y notificar a DrawControl
    if (label === 'Borrar') {
      if (drawRef) {
        const all = drawRef.getAll();
        if (all && all.features) {
          // Eliminar todas del mapa
          all.features.forEach((f) => drawRef.delete(f.id));
        }
      }
      dispatch(setMeasureMode('delete')); // modo especial para trigger delete en DrawControl
      
      // Reset inmediato
      setTimeout(() => {
        dispatch(setMeasureMode(null));
      }, 100);
      return;
    }

    // Medición - manejar ANTES del toggle general
    if (label === 'Distancia') {
      const willBeActive = !activeMapTool['Distancia'];
      
      setActiveMapTool({
        ...activeMapTool,
        'Distancia': willBeActive,
        'Área y perímetro': false
      });
      
      dispatch(setMeasureMode(willBeActive ? 'distance' : null));
      return;
    }

    if (label === 'Área y perímetro') {
      const willBeActive = !activeMapTool['Área y perímetro'];
      
      setActiveMapTool({
        ...activeMapTool,
        'Distancia': false,
        'Área y perímetro': willBeActive
      });
      
      dispatch(setMeasureMode(willBeActive ? 'area' : null));
      return;
    }

    // Toggle normal para otros botones
